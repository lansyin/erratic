...
fn from_error_round_trip() {
    let err = Error::from_error(TestError("oops"));
...
fn into_source_const_is_none() {
    let err = Error::from_context(literal!("test"));
    assert!(err.into_source().is_none());
}

#[test]
fn chain_wraps_source() {
    let inner = Error::from_error(TestError("inner"));
    let outer = Error::from_error(inner.erase());
